...
mobs_per_wave = {            -- Number of mobs spawned per wave, customizable per wave
	3,  -- Wave 1
        ...
        20, -- Wave 5
        40, -- Wave 6
        1, -- Wave 7-- The Final wave and the deadliest
},
...
local balrog_raid_data = {}

-- Function to spawn a wave of mobs around a player
local function spawn_mob_wave(player, wave_number)
    local player_pos = player:get_pos()
    local mob_count = 0
    for i = 1, balrog_raid_settings.mobs_per_wave[wave_number] do
        local spawn_pos = {
	x = player_pos.x + math.random(-balrog_raid_settings.spawn_radius, balrog_raid_settings.spawn_radius),
	y = player_pos.y,
	z = player_pos.z + math.random(-balrog_raid_settings.spawn_radius, balrog_raid_settings.spawn_radius)
        }
        local mob_type = balrog_raid_settings.mob_types[wave_number][math.random(1, #balrog_raid_settings.mob_types[wave_number])]
...
local function start_mob_raid(player)
    ...
    local function spawn_next_wave()
        if wave_number <= balrog_raid_settings.max_waves and player and player:is_player() and player:get_hp() > 0 then
	spawn_mob_wave(player, wave_number)
	wave_number = wave_number + 1
	minetest.after(balrog_raid_settings.wave_interval, spawn_next_wave)
        end
    end
    spawn_next_wave()
end
...
local function increase_difficulty()
    for i = 1, balrog_raid_settings.max_waves do
        balrog_raid_settings.mobs_per_wave[i] = balrog_raid_settings.mobs_per_wave[i] + math.floor(balrog_raid_settings.mobs_per_wave[i] * balrog_raid_settings.difficulty_scale)
    end
    minetest.after(balrog_raid_settings.difficulty_increase_interval, increase_difficulty)
end
]]
--[[ Function to reward players for surviving the raid
local function reward_players()

for player, _ in pairs(balrog_raid_data) do
        if player and player:is_player() then
           	local loot = {}

	for i = 1, balrog_raid_settings.max_waves do
                for _, item in ipairs(balrog_raid_settings.loot_table[i]) do
            ...
            for _, item in ipairs(balrog_raid_settings.reward_items) do
...
minetest.register_chatcommand("startraid2", {
   	params = "",
        ...
        if player then
	balrog_raid_data[player] = true
	start_mob_raid(player)
	return true, "Mob raid event started!"
        else
	return false, "You need to be in-game to start the raid."
        end
end,
})
...
-- increase_difficulty()
	minetest.after(balrog_raid_settings.wave_interval * balrog_raid_settings.max_waves, reward_players)
end)
...
--[[minetest.register_abm({
	...
	if not balrog_raid_data[player] then
		balrog_raid_data[player] = true
		start_mob_raid(player)
